...
@section('content')
    <div class="content">
        ...
        <div class="table-card">
            <div class="table-header"
                style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; gap: 20px;">
                ...
                <div class="table-header-right" style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <a href="{{ route('buku.create') }}" class="btn-add"
                        style="background: #7c3aff; color: #fff; padding: 9px 16px; border-radius: 8px; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 8px; font-size: 12px; transition: 0.3s;">
                    ...
                    <tbody>
                        @forelse($bukus as $index => $buku)
                            <tr style="border-bottom: 1px solid #f1f5f9; transition: 0.2s;"
                                onmouseover="this.style.backgroundColor='#fbf9ff'"
                                onmouseout="this.style.backgroundColor='transparent'">
                                <td style="text-align: center; color: #94a3b8; font-weight: 500;">
                                    {{ $bukus->firstItem() + $index }}</td>
                                ...
                                <td style="padding: 12px;">
                                    <div style="font-weight: 600; color: #1e1b3a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                                        title="{{ $buku->judul }}">
                                        {{ $buku->judul }}
                                    </div>
                                ...
                                <td style="text-align: center;">
                                    <div style="display: flex; gap: 6px; justify-content: center;">
                                        {{-- Tombol Detail (SUDAH JADI MATA) --}}
                                        <a href="{{ route('buku.show', $buku->id) }}"
                                            style="width: 28px; height: 28px; background: #f8fafc; color: #7c3aff; border: 1px solid #e2e8f0; border-radius: 6px; display: flex; align-items: center; justify-content: center; text-decoration: none; transition: 0.2s;"
                                            title="Detail"
                                            onmouseover="this.style.background='#7c3aff'; this.style.color='#fff'"
                                            onmouseout="this.style.background='#f8fafc'; this.style.color='#7c3aff'">
                                            <i class="fas fa-eye" style="font-size: 10px;"></i>
                                        </a>

                                        {{-- Tombol Edit --}}
                                        <a href="{{ route('buku.edit', $buku->id) }}"
                                            style="width: 28px; height: 28px; background: #eef2ff; color: #4338ca; border-radius: 6px; display: flex; align-items: center; justify-content: center; text-decoration: none; transition: 0.2s;"
                                            title="Edit" onmouseover="this.style.background='#4338ca'; this.style.color='#fff'"
                                            onmouseout="this.style.background='#eef2ff'; this.style.color='#4338ca'">
                                        ...
                                        <form action="{{ route('buku.destroy', $buku->id) }}" method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" onclick="return confirm('Yakin hapus data ini?')"
                                                style="width: 28px; height: 28px; background: #fff1f2; color: #e11d48; border-radius: 6px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.2s;"
                                                title="Hapus"
            ...
            <div class="pagination-row"
                style="display: flex; align-items: center; justify-content: space-between; margin-top: 25px; padding-top: 20px; border-top: 1px solid #f1f5f9;">
                <div style="font-size: 12px; color: #64748b;">
                    Showing <span style="font-weight: 600; color: #1e1b3a;">{{ $bukus->firstItem() ?? 0 }}</span> to <span
                        style="font-weight: 600; color: #1e1b3a;">{{ $bukus->lastItem() ?? 0 }}</span> of
                    {{ $bukus->total() }} entries
                </div>
    ...
    <style>
        .page-link-custom {
            ...
            width: 32px; height: 32px; text-decoration: none; color: #64748b;
            background: #fff; border: 1px solid #e2e8f0; border-radius: 8px;
            font-size: 12px; font-weight: 600; transition: 0.3s;
        }
        .page-link-custom:hover { background: #f1f5f9; color: #7c3aff; border-color: #7c3aff; }
        .page-link-custom.active { background: #7c3aff; color: #fff; border-color: #7c3aff; box-shadow: 0 4px 10px rgba(124, 58, 255, 0.2); }
        .page-item-disabled {
            display: flex; align-items: center; justify-content: center;
            width: 32px; height: 32px; color: #cbd5e1; background: #f8fafc;
            border: 1px solid #e2e8f0; border-radius: 8px; font-size: 11px; cursor: not-allowed;
        }
        .table-card { background: #fff; border-radius: 14px; padding: 24px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.05); }
    </style>
@endsection