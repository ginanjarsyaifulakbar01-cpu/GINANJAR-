...
@section('content')
    <style>
        ...
        .description-box h3 { font-size: 18px; font-weight: 800; margin-bottom: 15px; }
        .description-box p { color: #475569; line-height: 1.8; font-size: 16px; }
        ...
        <div class="detail-wrapper">
            ...
            <div class="detail-cover-wrapper">
                <div class="detail-img-container">
                    @php
                        ...
                        if (!$cover) {
                            ...
                            // Perbaikan Utama: Menghapus kata 'storage/' dari DB jika ada
                            // Supaya tidak terjadi URL ganda seperti storage/storage/
                            $cleanPath = str_replace('storage/', '', $cover);
                ...
                <div style="margin-top: 20px; padding: 20px; background: #f8fafc; border-radius: 20px; text-align: center;">
                    <span style="display: block; font-size: 13px; color: #64748b; margin-bottom: 5px; font-weight: 600;">
                        Status Ketersediaan
                    </span>
            ...
            <div class="detail-content">
                ...
                @if(session('success'))
                    <div style="background: #d1fae5; color: #065f46; padding: 20px; border-radius: 15px; margin-bottom: 25px; font-weight: 700; border: 1px solid #10b981; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <a href="{{ route('riwayat.pinjam') }}" style="background: #059669; color: white; padding: 8px 16px; border-radius: 10px; font-size: 13px; text-decoration: none;">
                            Cek Status <i class="fas fa-arrow-right"></i>
                        </a>
                    </div>
                @endif
                ...
                @if($statusPinjam)
                    <div style="background: {{ $isTerlambat ? '#fee2e2' : '#eff6ff' }}; border: 2px solid {{ $isTerlambat ? '#ef4444' : '#2563eb' }}; padding: 30px; border-radius: 24px; text-align: center;">
                        ...
                        <p style="color: {{ $isTerlambat ? '#991b1b' : '#64748b' }}; margin-bottom: 20px;">
                            Status Saat Ini: <strong>{{ strtoupper($statusPinjam->status) }}</strong>
                        </p>
                ...
                @elseif($buku->stok > 0)
                    <form action="{{ route('peminjaman.ajukan', $buku->id) }}" method="POST">
                        ...
                        <div class="form-peminjaman-box">
                            <div style="margin-bottom: 20px;">
                                <label class="info-label" style="color: #0f172a; font-size: 15px;">
                                    <i class="fas fa-calendar-day text-primary"></i> Rencana Tanggal Pinjam
                                </label>
                                <input type="date" name="tgl_pinjam" class="input-custom" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <div>
                                <label class="info-label" style="color: #0f172a; font-size: 15px;">
                                    <i class="fas fa-hourglass-half text-primary"></i> Durasi Peminjaman
                                </label>
                        ...
                        <button type="submit" class="btn-pinjam-full">
                            <i class="fas fa-book-reader"></i> Ajukan Peminjaman Sekarang
                        </button>
                    </form>
                @else
                    <div style="background: #f1f5f9; color: #64748b; padding: 25px; border-radius: 20px; text-align: center; font-weight: 700; border: 1px solid #e2e8f0;">
                        <i class="fas fa-info-circle"></i> Maaf, stok buku sedang habis.
                    </div>
                @endif
            </div>
        </div>
